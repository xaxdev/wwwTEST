const GetGemMatchSearch = require('../utils/getGemMatchSearch');

module.exports = (key, obj, userCurrency, clarity, cb) => {
    // const gemstoneFields = Object.keys(key);
    const valusObj = obj[key];
    const gemstoneFields = Object.keys(valusObj);
    // console.log('valusObj-->',valusObj);
    ...
    gemstoneFields.forEach((key)=>{
        ...
        if(key == 'clarity'){
            gemstoneFilterSplit.push(JSON.parse(GetGemMatchSearch('clarity',clarity)));
        }
        if(key == 'origin'){
    ...
    `{
      "nested": {
        ...
        "query": {
          "bool": {
            "must":
              ${JSON.stringify(gemstoneFilterSplit)}
          }
        }
      }
    }`;
