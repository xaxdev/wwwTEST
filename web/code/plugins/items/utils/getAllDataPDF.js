...
module.exports = async (response, sortDirections, sortBy, size, page, userCurrency, keys, obj, request,itemsOrder, setReferencdOrder, itemsNotMMECONSResult,itemsMMECONSResult, cb) => {
    try {
        let allData = [];
        let setReferences = [];
        let sumPriceData = [];
        ...
        data.forEach(function(item){
            if (isViewAsSet) {
                allData.push({
                    'id': item.id,'reference':item.reference,'createdDate':item.createdDate,'totalPrice':item.totalPrice,'description':item.description,
