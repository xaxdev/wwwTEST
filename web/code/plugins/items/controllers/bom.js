    ...
    handler: (request, reply) => {
        (async _ => {
            ...
            try {
                ...
                const { userName, allBom, ROOT_URL, company } = request.payload;
                const createTime = moment().format('YYYYMMDD_HHmmss');

                const cerFolder = Path.resolve(__dirname, '../../http/public/export_files/bom/');
                const userFolder = Path.resolve(__dirname, `../../http/public/export_files/bom/${userName}_${createTime}`);
                ...
                if (allBom.length > 1) {
                    ...
                    const emailBody = `Please download the files only by today from below link ${ROOT_URL}/export_files/bom/${userName}_${createTime}.zip.`;
                    ...
                    allBom.map((img) => {
                        (async _ => {
                            ...
                            destination = userFolder + '/' + img.replace('/images/products/original/','');
                            await copyFile(source,destination);
                            const emailBody = `Please download the files only by today from below link ${ROOT_URL}/export_files/bom/${userName}_${createTime}${img.replace('/images/products/original/','')}.`;
                            await notify('', emailBody, toEmail);
                        })()
                    });
                }
