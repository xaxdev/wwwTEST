...
import * as yingSetActions from '../../actions/yingsetaction';
import * as usersAction from '../../actions/usersaction';
import validateEmail from '../../utils/validateemail';

import '../../../public/css/react-multi-select.css';

class ShareModalYingCatalog extends Component {
    constructor(props) {
        super(props);
    ...
    componentWillMount = _=>{
        this.props.usersAction.fetchShareUsers();
    }
    openModal(){
    ...
    handleEmailSelectChange = (EmailSelectValue) =>{
        ...
        this.props.usersAction.setDataSendEmailTo(EmailSelectValue);
    }
    confirmDisableModal = () => {
        const { fields: { shareTo, validateEmailTo }, ShareEmailToValue, id } = this.props;
        this.setState({isOpen: false});
        let emails = [];
        let paramEmails = [];
        let params = {};
        ...
        this.props.yingSetActions.shareYingSet(params)
        .then((response)=>{
            this.setState({isOpenShareMyCatalog: false});
            this.props.usersAction.setDataSendEmailTo('');
            shareTo.onChange('');
            shareTo.value = '';
        })
    };
    render (){
        ...
        return(
            <div>
                <a>
                    <div className={`${false ? 'icon-share fa' : 'icon-share'}`}
                        onClick={false ? '' : this.openModal}></div>
                </a>
...
function mapDispatchToProps(dispatch){
    return {
        yingSetActions: bindActionCreators(Object.assign({}, yingSetActions), dispatch),
        usersAction: bindActionCreators(Object.assign({}, usersAction), dispatch)
    }
}
...
module.exports = reduxForm({
    form: 'ShareModalYingCatalog',
    fields: ['shareTo','validateEmailTo'],
    validate: validateEmail
},mapStateToProps,mapDispatchToProps)(ShareModalYingCatalog)
