            ...
            <div>
                {
                    this.props.items.map(function(item, index){
                        ...
                        if (item.id != null) {
                            ...
                            imagesProduct = (item.authorization)? (item.gallery.length) != 0? item.gallery[0].original: '/images/blank.gif' :'/images/login-logo@2x.png';
                            imagesProduct = (item.availability)? imagesProduct: '/images/imagesoldout@2x.png';
                            itemDate = (item.authorization)? (item.type != 'CER')? convertDate(item.itemCreatedDate): convertDate(item.itemCreatedDate) : '';
                            lblDate = (item.authorization) ? (item.type != 'CER')? 'Created Date:' : 'Certificate Date:': '';
                            price = (item.authorization)? (item.priceInUSD != -1)? numberFormat(item.priceInUSD) + ' ' + 'USD': '- ' + 'USD': '- ' + 'USD';
                            actualCost = (item.authorization)? (item.actualCostInUSD != -1)? numberFormat(item.actualCostInUSD) + ' ' + 'USD': '- ' + 'USD': '- ' + 'USD';
                            updatedCost = (item.authorization)? (item.updatedCostInUSD != -1)? numberFormat(item.updatedCostInUSD) + ' ' + 'USD': '- ' + 'USD': '- ' + 'USD';
                            itemName = (item.authorization)? (item.type != 'CER')?(item.description != undefined) ?
                                            (item.description.length <= 80) ? item.description : item.description.substring(0, 80) + '...': '-' :item.name: '';
                            ...
                            itemDate = (item.authorization)? (item.type != 'CER') ? convertDate(item.createdDate) : convertDate(item.createdDate): '';
                            lblDate = (item.authorization)? (item.type != 'CER') ? 'Created Date:' : 'Certificate Date:': '';
                            price = (item.authorization)? (item.totalPrice['USD'] != -1)? numberFormat(item.totalPrice['USD']) + ' ' + 'USD': '- ' + 'USD': '- ' + 'USD';
                                    ...
                                    <div className="pull-right">
                                        <div className={`${isCatalogShared ? 'hidden' : 'checkbox checkbox-warning'}`}>
                                            <input type="checkbox" className="styled"
                                                ...
                                                id={index} checked={checkItem} onChange={onCheckedOneItemMyCatalog} />
                                        ...
                                        <div className={`${isCatalogShared ? 'hidden' : 'grid-add margin-r5'}`}>
                                            <span className="icon-det-28"
                                                name={item.id != null ? `id=${item.id}` : `reference=${item.reference}`}
                                                value={item.id != null ? `id=${item.id}` : `reference=${item.reference}`}
                                                id={index} onClick={onDeleteOneItemMyCatalog}></span>
                                        </div>
