...
class SetCatalog extends Component {
    constructor(props) {
        ...
        this.state = {
            activePage: !!this.props.currentPage?this.props.currentPage:1, isOpenDeleteItem: false, isOpenAddMyCatalogmsg: false, isTooltipActive: false, 
            isOpenDeleteCatalog: false, enabledMyCatalog: false, isOpenDeleteAllItem: false, isOpenZeroCatalog: true, isOpenPrintPdfmsg: false, 
            isOpenShareMyCatalog: false, checkAllItems: false, isOpenPrintOptions: false, isOpenWordOptions: false, allFields: false, groupCost: false, 
            updatedCost: false, sellingCost: false
        }
    }
    ...
    printResults(e){
        ...
        let dv = {
            'dvTotalItems': dvTotalItems, 'dvTotalSetItems': dvTotalSetItems, 'dvGridview': dvGridview, 'printPage':printPage, 'printPrice': printPrice, 
            'items': items, 'userLogin': userLogin, 'env': env_web
        };
        let htmlTemplate = '';
    ...
    confirmExport = _ =>{
        ...
        let params = {
            'temp': htmlTemplate, 'userName': `${userLogin.username}_${exportDate}`, 'userEmail': userLogin.email, 'ROOT_URL': ROOT_URL, 'channel':'word'
        };
        this.props.writeHtml(params).then((value) => {
            if (value) {
                this.setState({isOpenPrintPdfmsg: true});
    ...
    renderDialogPrintOptions = _ =>{
        ...
        return(
            <ModalPrintOptions onSubmit={this.printResults} isOpen={this.state.isOpenPrintOptions} isClose={this.handleClosePrintOptions} props={this.props} />
        );
    }
    ...
    renderModalConfirmDeleteAllItem = _=> {
        ...
        return(
            <ModalConfirmDelete onSubmit={this.handleSubmitDeleteAllItem} isOpen={this.state.isOpenDeleteAllItem} isClose={this.handleCloseDeleteAllItem}
                props={this.props} message={msg} title={title}/>
        );
    }
    ...
    renderModalConfirmDeleteCatalog = _=> {
        ...
        return(
            <ModalConfirmDelete onSubmit={this.handleSubmitDeleteSetCatalog} isOpen={this.state.isOpenDeleteCatalog} isClose={this.handleCloseDeleteCatalog} 
                props={this.props} message={msg} title={title}/>
        );
    }

    renderTotals(){
        const { listSetCatalogItems,totalPrice,totalUpdatedCost } = this.props;
        const { setItemPrice,setItemUpdatedCost } = listSetCatalogItems;
    ...
    renderAlertmsgShareSetCatalog = _=> {
        ...
        return(
            <Modalalertmsg isOpen={isOpen} isClose={this.handleClosemsgShareSetCatalog} props={this.props} message={shareSetCatalogmsgError}  title={title}/>
        );
    }
    ...
    render() {
        const {  setCatalogId, setCatalogName, isCatalogShared, ViewAsSet } = this.props;
