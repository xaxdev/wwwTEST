...
export default function(state = INITIAL_STATE, action){
    switch(action.type){
        case LOGIN_USER:
            ...
            return {...state, loginstatus: (action.data.statusCode >= 400) ? false : true, logindata:(action.data.statusCode >= 400) ? null : action.data,
        ...
        case RESETPASSWORD_USER:
            ...
            return {...state, loginstatus: (action.data.statusCode >= 400) ? false : true,
                logindata:(action.data.statusCode >= 400) ? null : action.data.data, msg:action.data.message}
        case VALIDATE_USER:
            if(action.data){
                sessionStorage.setItem('logindata', JSON.stringify(action.data));
            }
            return {...state, loginstatus: (action.data.statusCode >= 400) ? false : true,
                logindata:(action.data.statusCode >= 400) ? null : action.data, msg:''}
        case LOGOUT_USER:
        ...
        default:
            ...
            return {...state,loginstatus: token ? true : false, logindata:token ? logindata : null, msg:''}
    }
}
