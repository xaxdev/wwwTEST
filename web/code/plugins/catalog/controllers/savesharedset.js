    ...
    handler: (request, reply) => {
        ...
        (async () => {
            ...
            try {
                ...
                const setCatalogId = request.payload.id;
                ...
                const findShared = await db.collection('SetCatalogShared').findOne(
                    {
                        'setCatalogId': new ObjectID(setCatalogId),
                        'owner': owner.id
                    });
                ...
                const updatedShared = await db.collection('SetCatalogShared').updateOne(
                    {
                        'setCatalogId': new ObjectID(setCatalogId),
                        'owner': owner.id
                    },
                ...
                return reply({error:'',message:'Share set catalog success.',statusCode:200});
            } catch (e) {
