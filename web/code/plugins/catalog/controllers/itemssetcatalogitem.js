    ...
    handler: (request, reply) => {
        (async _ => {
            ...
            try {
                ...
                const setCatalogId = request.mongo.ObjectID(id)
                ...
                const cursor = await request.mongo.db.collection('SetCatalogName').aggregate([
                    {
                        $match: { _id: setCatalogId }
                    },
                    {
                        $lookup: {
                            from: 'SetCatalogItem',
                ...
                const [setCatalog] = await cursor.toArray()

                if (!!setCatalog) {

                    if (setCatalog.items.length === 0) {
                        return reply({ ...setCatalog, page: 1, total_items: 0, total_pages: 0, })
                    }
                    ...
                    const data = await request.mongo.db.collection('SetCatalogItem').find(
                        {
                            setCatalogId
                        })
                        .sort(sorting)
                        .toArray()
                    const dataSorted = await request.mongo.db.collection('SetCatalogItem').find(
                        {
                            setCatalogId
                        })
                        .sort(sorting)
                    ...
                    return reply({ ...setCatalog, price, updatedCost, setItemPrice, setItemUpdatedCost, page,
                        total_items, total_pages, total_setitems, 'items': response, 'allItems': responseAllItems })
                }

                return reply(Boom.badRequest('Invalid set catalog id'))
            } catch (e) {
